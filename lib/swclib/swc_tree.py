...
from lib.swclib import swc_node
...
class SwcTree:
    ...
    def load_matrix(self, path):
        ...
        with open(path, 'r') as fp:
            ...
            k = 0
            line_first = lines[0]
            if self.is_comment(line_first):
                swc_data = np.zeros([len(lines) - 1, 7])
            else:
    ...
    def swc_save_preorder(self, outpath):
        with open(outpath, 'w') as fp:
            ...
            for tn in niter:
                ...
                # fp.write("{} {} {} {} {} {} {}\n".format(node_id_temp, node._type,node.get_x(),node.get_y(),node.get_z(),node.radius(),node.parent.get_id(),))
    ...
    def rescale(self, scale):
        for node in self.get_node_list():
            ...
            node.set_r(node._radius * scale[3])
    
    def relocation(self, scale):
        for node in self.get_node_list():
            node.set_z(node.get_z() + scale[0])
            node.set_y(node.get_y() + scale[2])
            node.set_x(node.get_x() + scale[1])
...
if __name__ == "__main__":
    ...
    b_tree = tree.break_branches()
    from pyneval.pyneval_io import swc_io
>>>>>>> 20bdc2c42733a49951a9d875f2000067f89892b7
